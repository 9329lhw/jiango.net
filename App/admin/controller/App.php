...
class App extends Base
{
    public function index()
    {
        ...
        $this->assign('action_link', array('text' => '添加新版本', 'href' => '/admin/app/editApp'));
        return $this->display();
    }
    ...
    public function saveApp()
    {
        ...
        if($device && $status){
            $info = model('AppSeting')->where(['device'=>$device,'status'=>$status])->find();
        }
        if(!empty($info)){
            return $this->url_redirect($device.'已经设置了，请先修改原先的版本信息', "/admin/app/index", "App版本管理");
        }
        ...
        return $this->url_redirect($result, "/admin/app/index", "App版本管理");
