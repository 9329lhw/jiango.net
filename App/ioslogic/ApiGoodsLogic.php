...
class ApiGoodsLogic extends Base {
     ...
     * @return string
     */
    public function getGoodsList($page,$page_size,$coupon_rate, $update_time) {
        $order = '';
        if(!empty($sort)){
            $order .= "$sort desc";
        }
...
//            \lib\cache\RedisCache::instance()->lPush('tb_coupon_queue',$coupon_queue);
            ...
            foreach ($tb_array_coupon as $key=>$val){
                ...
                if ($val['volume'] > 0) {
                    ...
                    $goods['copy_text'] = $val['item_description'];
                    $goods['nick'] = $val['nick'];
                    ...
                    $goods['zhekou'] = number_format(($val['zk_final_price'] / $val['reserve_price']) * 10, 2);
