...
//        $permissionlist = $actionobj->getActionTree();
    ...
    public function editAction(){error_reporting(E_ERROR | E_PARSE);
        ...
        $this->assign('action_link', array('text' => '权限列表', 'href' => '/tebiedemingzi/Permission/index'));
        return $this->display();
    }
    ...
    public function saveAction(){
        ...
        return $this->url_redirect($result, "/tebiedemingzi/permission/index", "权限列表");
    }
    ...
    public function deleteAction(){
        $id = input('id','');
        if(empty($id)){
            $this->redirect('/tebiedemingzi/permission','',2);
        }else{
            $result = model('AdminAction')->deleteAction($id);
            if($result){
                return $this->url_redirect("删除成功", "/tebiedemingzi/permission", "权限列表");
            }else{
                return $this->url_redirect("删除失败", "/tebiedemingzi/permission", "权限列表");
            }
        }
    }
}
