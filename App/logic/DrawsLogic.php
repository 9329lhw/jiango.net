...
class DrawsLogic extends Base {
    ...
    public function getDrawsList($uid,$page,$page_size) {
        ...
        $total = Draws::where(['uid'=>$uid])->count();
        $total_page = ceil($total / $page_size);
        if($page > $total_page){
            $logs = [
                'list'=>$total_page?Draws::getDrawsList($uid,$total_page,$page_size):[],
...
//            Result::instance()->fail('我是有底线的')->output();
            ...
            $logs = [
                'list'=>Draws::getDrawsList($uid,$pre,$page_size),
                'total_page' => $total_page,
                'cur_page' =>$page
            ];
        }
    ...
    public function checkDraws($uid,$tel,$real_name,$aplay_number) {
     ...
     public function getAplayInfo($uid) {
        ...
        return [
            ...
            'undraws_point' => $userinfo['point'] - $userinfo['enabled_point'],
