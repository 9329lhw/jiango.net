...
//        $items = TbOrdersItem::where(['pic_url'=>''])->order('id')->group('num_iid')->column('num_iid');
        $num_iids = array();
        foreach($items as $v){
            $pic_url = db('dtk_goods')->where('num_iid',$v)->value('pic_url');
            if(!$pic_url){
                $pic_url = db('tb_coupon_goods')->where('num_iid',$v)->value('pic_url');
            }
            if(!$pic_url){
